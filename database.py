...
# Get CockroachDB connection string (e.g. postgresql://user:pass@free-tier.cockroachlabs.cloud:26257/defaultdb?sslmode=verify-full)
DATABASE_URL = os.environ.get("DATABASE_URL")


def get_connection():
    """Returns a connection to CockroachDB using dictionary-like row formatting."""
    if not DATABASE_URL:
...
def init_db():
    """Initializes CockroachDB database tables for employees and tasks."""
    with get_connection() as conn:
        with conn.cursor() as cursor:
            # Employees table schema
            # Note: CockroachDB uses 64-bit integer IDs (unique_rowid()) or SERIAL to avoid hotspotting
            cursor.execute("""
            ...
            """)
            conn.commit()
    logger.info("CockroachDB tables initialized successfully.")
